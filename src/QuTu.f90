! =============================================================================
! Program: QuTu - Quantum Tunnelling Simulator
...
!
! <v0> Creation date: 28/05/2022
! <v1.0> Update 20/01/2026:
!        - Refactored code (modules, types)
!        - Unified INPUT file reading
! <v2.0> Update 27/02/2026:
!        - Renamed to QuTu
!        - Added --help / -h flag to print reference INPUT
! =============================================================================
program QuTu
    use constants
    use types
    ...
    integer :: i, j, ierr, N_loop, exit_stat, cmd_stat
    integer :: n_steps_x, n_alpha
    real(dp) :: x_val

    ! Early-exit convergence control
    integer  :: N_exit
    logical  :: converged, do_output
    real(dp) :: delta_E_conv
    ...
    ! Maximum number of energy levels for convergence analysis
    integer, parameter :: N_CONV_MAX = 21
    integer :: n_conv_levels
    ...
    ! Check for -h / --help flag and print reference INPUT if requested
    block
        integer :: nargs
    ...
    if (cmd_stat /= 0 .or. exit_stat /= 0) stop "ERROR: Cannot create output directory"
    ...
    write(*,'(A,ES10.3,A)') ' Conv. threshold = ', input_params%conv_threshold, ' Ha'
    write(*,'(A,L1)')       ' Force full loop  = ', input_params%force_full_loop
    write(*,'(A)') '=================================================='
    ...
    ! n_conv_levels needed both inside the loop (convergence check) and after
    n_conv_levels = min(N_CONV_MAX, N_max)
    ...
    do N_loop = 2, N_max
        ...
        ! ---------------------------------------------------------------------
        ! Convergence check: monitor highest tracked level (converges last)
        ! ---------------------------------------------------------------------
        do_output = (params%N == N_max)

        if (.not. input_params%force_full_loop .and. .not. do_output .and. &
            params%N > n_conv_levels) then
            ! Compare highest tracked level between current and previous iteration
            delta_E_conv = abs(E_arr(params%N - 1, n_conv_levels) - &
                               E_arr(params%N - 2, n_conv_levels))
            if (delta_E_conv < input_params%conv_threshold) then
                converged = .true.
                do_output = .true.
                N_exit    = params%N
                write(*,'(A)') ' =================================================='
                write(*,'(A)') ' CONVERGENCE REACHED'
                write(*,'(A,I4,A,I4)')    '  N_exit = ', N_exit, '  (N_max = ', N_max, ')'
                write(*,'(A,ES10.3,A,I0,A)') &
                    '  |dE|   = ', delta_E_conv, ' Ha  (level ', n_conv_levels, ')'
                write(*,'(A)') ' =================================================='
            end if
        end if
        ! ---------------------------------------------------------------------

        ! Compute and save all results when output is triggered
        if (do_output) then
            ! Store final energies
            allocate(E_all_cm(N_max))
            E_all_cm = E_all * EUACM  ! convert to cm-1

            ! Write all eigenvalues to file
            call write_energies(trim(data_dir)//"out-energias_hartrees.dat", &
                                E_all, "Ha", ierr=ierr)
            call write_energies(trim(data_dir)//"out-energias_cm-1.dat", &
                                E_all_cm, "cm-1", ierr=ierr)
            ...
            ! -----------------------------------------------------------------
            if (input_params%n_alpha > 0) then
                n_alpha = input_params%n_alpha
                allocate(alpha_4EE(n_alpha))
                alpha_4EE = input_params%alpha_4EE
            else
                n_alpha = 13
                allocate(alpha_4EE(n_alpha))
        ...
        ! Exit loop early if convergence was detected
        if (converged) exit
    end do
    ! =========================================================================
    ...
    ! CONVERGENCE ANALYSIS
    ! =========================================================================
    ! Analyze convergence for the first N_CONV_MAX levels, capped by actual N_exit
    n_conv_levels = min(N_CONV_MAX, N_exit)
    allocate(N_conv(n_conv_levels), E_conv(n_conv_levels), E_max(n_conv_levels))

    do j = 1, n_conv_levels
        do i = 1, N_exit - 1
            E_sup = E_arr(N_exit - 1, j)
            E_inf = E_arr(i, j)
            delta_E = E_inf - E_sup
    ...
    call write_convergence(trim(data_dir)//"out-conver_energias_cm-1.dat", &
                           n_conv_levels, N_conv, E_conv, E_max, "cm-1", ierr)

    ! Write N vs W files for first 4 levels
    call write_N_vs_W_files(data_dir, E_arr, N_exit, N_conv, ierr)
...
contains
    ! Orchestration subroutines: kept here because each coordinates several
    ! modules and tightly couples computation with I/O for this output format.
    ...
    subroutine write_N_vs_W_files(data_dir, E_arr, N_max, N_conv, ierr)
        ...
        do level = 0, min(3, size(N_conv) - 1)
    ...
    ! =========================================================================
    ! SUBROUTINE: print_input_template
    ! Prints a fully commented reference INPUT file to stdout.
    ! Usage: ./QuTu -h > INPUT
    ! =========================================================================
    subroutine print_input_template()
        write(*,'(A)') '# ============================================================'
        write(*,'(A)') '# QuTu - Quantum Tunnelling Simulator'
        write(*,'(A)') '# Reference INPUT file generated by: QuTu --help'
        write(*,'(A)') '# Usage: ./QuTu  (reads INPUT from current directory)'
        write(*,'(A)') '# ============================================================'
        write(*,'(A)') ''
        write(*,'(A)') '# --- Basis Set ---'
        write(*,'(A)') 'N_max = 200          # [---] Max basis functions (positive even integer)'
        write(*,'(A)') ''
        write(*,'(A)') '# --- Potential Parameters ---'
        write(*,'(A)') 'xe = 0.3816          # [Ang] Equilibrium position (NH3 inversion)'
        write(*,'(A)') 'Vb = 2028.6          # [cm-1] Barrier height'
        write(*,'(A)') ''
        write(*,'(A)') '# --- Atomic Masses ---'
        write(*,'(A)') 'mass_H = 1.00782503207   # [amu]'
        write(*,'(A)') 'mass_N = 14.0030740048   # [amu]'
        write(*,'(A)') ''
        write(*,'(A)') '# --- Spatial Grid ---'
        write(*,'(A)') 'xmin = -5.0          # [a0]'
        write(*,'(A)') 'xmax =  5.0          # [a0]'
        write(*,'(A)') 'dx   =  0.02         # [a0]'
        write(*,'(A)') ''
        write(*,'(A)') '# --- Convergence (optional) ---'
        write(*,'(A)') 'conv_threshold  = 1.0e-8   # [Ha] Loop exit threshold'
        write(*,'(A)') 'force_full_loop = .false.  # Run to N_max even if converged'
        write(*,'(A)') ''
        write(*,'(A)') '# --- 4-state wavepackets (optional) ---'
        write(*,'(A)') '# alpha_4EE = 0.0, 10.0, 45.0, 90.0   # [deg] Mixing angles'
    end subroutine print_input_template
